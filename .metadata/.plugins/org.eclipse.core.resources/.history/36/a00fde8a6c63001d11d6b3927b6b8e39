...
public class dataDriven {
	...
	public ArrayList<String> getData(String testcaseName) throws IOException {
		//Identify testcases column by scanning the entire 1st row
		   ...
		   //after identifying purchase test case row, pull all the data of that row and feed it into test
		ArrayList<String> a = new ArrayList<String>();
		FileInputStream fis  = new FileInputStream("C://work//demodata.xlsx");
		XSSFWorkbook workbook = new XSSFWorkbook(fis);  //XSSFWorkbook only accepts file input stream argument so fis is given as an argument to it
				...
				while(rows.hasNext()) {
					...
					if(r.getCell(column).getStringCellValue().equalsIgnoreCase(testcaseName)) {
						...
						if(cv.next().getCellType()==CellType.STRING) {
								a.add(cv.next().getStringCellValue()); //a is an ArrayList of String declared in line 22 in which the columns values are getting added
						}else {
